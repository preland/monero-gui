...
//^base url to use is https://github.com/PurpleI2P/i2pd/releases/recent; recent
// autoresolves to tag/<version> download i2pd version matching the platform and
// version ^take base url and add "i2pd_<version><platform tag>" ^should support
// win64, macx86/macarm, linx64 download checksum and compare if applicable
//
// function to launch i2pd with some given flags
// some sort of async thread thingy to run i2pd headless---should probably
// remove browser viewer by default
//
...
std::string I2PManager::getFlags(int mode) {
  ...
  case 1: // simple (bootstrap)
    break;
  case 2: // advanced
    monerod_args += std::string("--proxy=127.0.0.1:4447") +
                    "--daemon-address=" + getAddress(5) + "--trusted-daemon";
    break;
  default:
    break;
  }
...
bool I2PManager::start(const QString &flags) {
  ...
  QStringList arguments = QStringList() << "--conf" << m_i2pd_config
  ...
  connect(m_daemon.get(), SIGNAL(readyReadStandardOutput()), this,
          SLOT(printOutput()));
  connect(m_daemon.get(), SIGNAL(readyReadStandardError()), this,
          SLOT(printError()));
  ...
  connect(m_daemon.get(), SIGNAL(stateChanged(QProcess::ProcessState)), this,
  ...
  m_scheduler.run([this] {
    if (startWatcher()) {
      ...
      emit daemonStartFailure(
          tr("Timed out, local node is not responding after %1 seconds")
              .arg(DAEMON_START_TIMEOUT_SECONDS));
    }
  });
...
bool I2PManager::startWatcher() const { return false; }
bool I2PManager::stopWatcher() const { return false; }
...
bool I2PManager::I2PInstall() {
  ...
  std::string download_folder =
      QStandardPaths::writableLocation(QStandardPaths::DataLocation)
          .toStdString() /*QApplication::applicationDirPath().toStdString()*/
      + "/i2pd/downloaded";
  ...
  /*}*/
  std::string url =
      get_url_redirect("https://github.com/PurpleI2P/i2pd/releases/latest");
  ...
  std::string downloadurl =
      "https://www.github.com/PurpleI2P/i2pd/releases/download/" + version +
      "/" + filename;
  ...
  if (curl) {
    ...
    if (fp) {
      ...
      qCritical() << "Error opening file "; // + outfilename;
    }
  }
  curl_easy_cleanup(curl);
  curl_global_cleanup();
  std::cout << "9" << std::endl;
  // sanitize outfilename, because of macOS
  // std::string tmp;
  ...
  std::string tarCmd = "tar -xf data.tar.xz -C " +
  ...
  tools::copy_file(QApplication::applicationDirPath().toStdString() +
                       "/i2pd/downloaded/usr/bin/i2pd",
                   QApplication::applicationDirPath().toStdString() +
                       "/i2pd/i2pd");
  // add config files: could be downloaded from repo?
  write_file(
      QApplication::applicationDirPath().toStdString() + "/i2pd/i2pd.conf", "\
loglevel = none\n\
ipv4 = true\n\
ipv6 = false\n\
[ntcp2]\n\
...
[sam]\n\
enabled = false");
  write_file(QApplication::applicationDirPath().toStdString() +
                 "/i2pd/tunnels.conf",
             "\
[monero-node]\n\
type = server\n\
...
int is_server_online(const char *url) {
  ...
  curl_easy_setopt(curl, CURLOPT_FAILONERROR,
                   1L); // Treat HTTP errors as failures
...
std::string I2PManager::getAddress(int max_retries) {
  ...
  // std::string outfilename = m_i2pd_download_dir.toStdString() +
  // m_sep.toStdString() + "idkwhatthisis"; std::cout << outfilename <<
  // std::endl;
  ...
  if (curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT,
                     10L); // Transfer timeout: 10 seconds
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT,
                     5L); // Connection timeout: 5 seconds
  ...
  if (startPos == std::string::npos)
    return "failed@curl_parse1";
  startPos += prefix.length();
  size_t endPos = readBuffer.find(suffix);
  if (endPos == std::string::npos)
    return "failed@curl_parse2";
...
I2PManager::I2PManager(QObject *parent) : QObject(parent), m_scheduler(this) {
